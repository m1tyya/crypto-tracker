...
export function from(rawColor: Color): ColorOutput {
	...
	const outputs = {
		get hex(): string {
			...
			if (color[3] < 1) {
				hexString += leftPad(Math.round(a * 255).toString(16), 2);
			} // a
		...
		get hexVal(): number {
			let [r, g, b, a] = color;
			if (a < 1) {
				console.warn(`hexVal converted a semi-transparent color (${a * 100}%) to fully opaque`);
...
function parseValueStr(colorStr: string, normalize?: Array<number>): [number, number, number, number] {
	...
	for (const [n, value] of matches.entries()) {
...
export function parse(rawColor: Color): sRGB {
	...
	if (typeof rawColor == 'string') {
		...
		if (!strVal) {
			throw new Error(`Expected color, received empty string`);
		}
